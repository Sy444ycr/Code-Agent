...
2026-08-14 在用户指定的隔离 worktree `C:\Users\sy444\Desktop\Agents\.worktrees\task-25-restart-recovery` 串行处理 Task 25 最终再复审的两个 Important。既有 Task 25 SDD 与用户给出的最终审查项作为已批准规格；因用户要求只提交一个 fix commit 且修改范围不包含新的设计/计划文件，本轮没有创建额外规格文档。

第一项严格按 TDD 扩展 submit 失败测试：测试先写入可见游标，在注入的 `executor.submit` 失败函数内部读取其后的事件。红灯实际读到 sequence 2 的 `recovery_started`，证明旧补偿虽然最终删除事件，但失败窗口仍可对外暴露伪事件并造成客户端游标跨过后续复用 sequence。最小实现让 `claim_recovery` 的 `BEGIN IMMEDIATE` 事务只原子 claim task/recovery 和拒绝旧 approvals；`_start_runtime` 向 executor 提交受启动门闩阻塞的 worker，提交成功后追加 `recovery_started`，随后才放行 worker。submit 失败时移除 runtime，storage 只恢复 `needs_review + recovery.required=True`，不删除事件；旧 approvals 保持 `rejected`。绿灯同时覆盖失败窗口、失败后历史、成功重试的连续 sequence，以及从失败前游标读取 `recovery_started` 与 `task_completed`。

第二项用真实 TaskManager runtime 构造确定性红灯：PLAN 模式的 shell 决策使 worker 得到 `needs_review`，测试将它阻塞在 `task_completed` 追加前，再读取 SSE。旧实现因对 `needs_review` 无条件提前返回而没有调用 `wait_for_event`。最小修复移除该提前返回：活动 runtime 继续有限等待并回放完成事件；重启隔离形成的无 runtime `needs_review` 仍由 `wait_for_event` 的 `KeyError` 立即关闭；`after == task_completed.sequence` 仍通过持久化完成事件回查立即关闭。新增测试转绿，三项既有边界回归同时通过。

目标集成测试为 `36 passed, 41 warnings`，触碰 Python 文件的 Ruff 检查通过。最终提交门禁继续执行 Python 全量、`ruff check .`、`mypy src`、Web Vitest 与 Web build；两份 `task-5-report.md` 保持相同内容。既有 FastAPI `on_event`、Starlette TestClient/httpx 弃用提示与 Vite `configLoader: 'native'` 迁移预警不影响退出状态。当前环境无通用 subagent 调度工具，因此无法执行独立 reviewer，改为按审查模板逐项自审并在最终 concern 中如实记录。
